...
class MFCC(object):
    def __init__(self, nfilt=40, ncep=13,
                 ...
                 samprate=16000, frate=100, wlen=448,
                 nfft=512):
        # Store parameters
        self.lowerf = lowerf
        self.upperf = upperf
        ...
        self.wlen = wlen # in samples per second
...
#             print self.filters[leftfr:rightfr,whichfilt]
    ...
    def sig2s2mfc(self, sig):
        ...
        fr = 0
        frame = sig[0:self.wlen]
        frame = self.pre_emphasis(frame) * self.win
        fft = numpy.fft.rfft(frame, self.nfft)
        # Square of absolute value
        power = fft.real * fft.real + fft.imag * fft.imag
        power=numpy.clip(power[:self.nfft//2],1e-5,numpy.inf)
        #print("row row",self.wlen,power)
...
class MFCCProcess(SimpleProcess):

    def run(self):

        mfcc = MFCC(samprate=self.signal.sample_rate)
        dat = mfcc.sig2s2mfc(self.signal.data)

        return Matrix(dat,mfcc.frate)
...
def main():
    sr,sig = SigProc.WaveWRX().read("cnn.wav",dtype=numpy.float32)
    print(sig.shape)

    mfcc = MFCC(samprate=sr)

    dat = mfcc.sig2s2mfc(sig)
